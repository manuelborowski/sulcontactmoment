...
$(document).ready(function () {
    ...
    var datatable_config = {
        ...
        drawCallback: function (settings) {
            if("width" in table_config) {$table.attr("width", table_config.width);}
            busy_indication_off();
            if (cell_to_color) {
    ...
    datatable_dom_buttons = []
    datatable_dom = "fiptlp";
    if (table_config.export_excel_button) {
        datatable_dom_buttons.push("excelHtml5");
        table_config.suppress_dom = false;
        datatable_dom = "Bfiptlp";
    }
    if (table_config.suppress_dom) {
        datatable_config["filter"] = false;
        datatable_config["paging"] = false;
    } else {
        datatable_config["dom"] = datatable_dom;
        datatable_config["buttons"] = datatable_dom_buttons;
    }
